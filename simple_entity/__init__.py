...
class ValueObject(metaclass=DataclassMeta):
    ...
    def from_dict(cls, data: dict):
        return cls(**__get_obj_dict__(data, cls)) if data else None
...
class Entity(ValueObject):
    _id: str = field(default_factory=gen_uuid)
    ...
    for key, value in data.items():
        if isinstance(annotations[key], _GenericAlias):
            if issubclass(annotations[key].__args__[0], ValueObject):
                obj_dict[key] = [
                    annotations[key].__args__[0].from_dict(item)
                    for item in value
                ]
            else:
                obj_dict[key] = value
...
def method_call_log(obj, method_name: str):
    def outter(func):
        def inner(*args, **kwargs):
            ...
            logger.info(f'Call method "{method_name}" of {obj} with args {args} kwargs {kwargs} returns {ret}')
            return ret
